...
{
	0x29, 0x23, 0xBE, 0x00, 0xE1, 0x6C, 0xD6, 0xAE, 0x52, 0x90,
...
WZAG_API int AGRequestData(int NoneUse, int GetType)
{
    ...
    fstream1 = fopen(filename, "rb");

    if(fstream1)
    {
        fseek(fstream1, 0, SEEK_END);
        FileLen = ftell(fstream1);
		...
		{
			MessageBox(NULL,filename, "Impossivel ler o arquivo",MB_OK | MB_ICONERROR);
            ::ExitProcess(0);
        }
    ...
    {
        MessageBox(NULL, filename, "Impossivel abrir o arquivo",MB_OK | MB_ICONERROR);
        ::ExitProcess(0);
    }
    return 0;
}
...
WZAG_API int AGSetInfo(int Language, char * Partition, int un0, char * SrvVer, char* SrvName, int SrvType, char * Addr7)
{
    switch(Language)
	{
		...
		default:
		{
			MessageBoxA(0,"Linguagem escolhida nao suportada.\nAltere em CommonServer.cfg","Erro de linguagem", MB_OK | MB_ICONERROR);
			::ExitProcess(0);
