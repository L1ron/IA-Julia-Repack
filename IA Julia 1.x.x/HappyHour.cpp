...
void cHappyHour::ResetConfig()
{
    ...
    for (int i=0; i<128; i++)
    {
        HappyStruct[i].MapNum		= 0;
        HappyStruct[i].P_Zen		= 0;
        HappyStruct[i].P_Drop		= 0;
        HappyStruct[i].P_Exp		= 0;
        HappyStruct[i].S_WeekDay	= 0;
        HappyStruct[i].S_Hour		= 0;
        HappyStruct[i].S_Min		= 0;
        HappyStruct[i].T_Delay		= 0;

        HappyStruct[i].C_WeekDay	= 0;
        HappyStruct[i].C_Hour		= 0;
        HappyStruct[i].C_Min		= 0;
        HappyStruct[i].Started		= 0;
    }

    for (int i=0; i<64; i++)
    {
        MessageTimeBefore[i]	= 0;
        MessageTimeAfter[i]		= 0;
    }
}
...
void cHappyHour::Load()
{
    ...
    {
        HappyHourEnabled = 0;
        Log.ConsoleOutPut(1, c_Red, t_Error, "[X] [HappyHour]\tCan`t Find %s", IAJuliaHappyHour);
        return;
    }
    ...
    Log.ConsoleOutPut(1, c_Cyan, t_Default, "[û] [Happy Hour]\tLoaded %d locations." ,HappyHoursNumber);
}
