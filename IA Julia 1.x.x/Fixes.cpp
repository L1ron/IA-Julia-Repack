...
void cFixes::ASMFixes()
{
...
#ifdef _GS
    // ПРОВЕРЕН (GS,GSCS)
    BYTE IOCPResponceErrorJump[] = {0xE9, 0xC3, 0x02, 0x00, 0x00, 0x90, 0x90, 0x90, 0x90}; //JMP 004B2066
    memcpy((int*)0x004B1D9E, IOCPResponceErrorJump, sizeof(IOCPResponceErrorJump));

    BYTE IOCPAdd79Error[] = 
    {
        0x81, 0x7D, 0xD0, 0xE3, 0x03, 0x00, 0x00, 0x0F, 0x84, 0x66, 0xFD, 0xFF,
    ...
    //Hook JGPGetCharInfoEx - ПРОВЕРЕН (GS,GSCS)
    BYTE CharWingsEx[] = 
    {
        0x8D, 0xBD, 0xEC, 0xFE, 0xFF, 0xFF, 0x57, 0xFF,
    ...
    *(unsigned int*)0x0071B200  = (unsigned int)JGPGetCharInfoEx;
    ...
    // Это фиксы ссылок на оффсеты (все сдвинулось из за хука JGPGetCharInfoEx)
    Utilits.SetByte(0x0042B03A+1,0xA5); //небыло, добавил сам, но он 99.99% верный
    Utilits.SetByte(0x0042B0C8+2,0xD5); //был уже, просто короче сделал
    Utilits.SetByte(0x0042B0FA+2,0xA3); //был уже, просто короче сделал

    /*
    Beos: хук исправил (краш) но это скорее всего ошибочный хук т.к. в нем нет смысла + ОЧЕНЬ похож на опечатку
    OLD: 0042B032  |. E8 56C8FDFF    CALL GameServ.0040788D		-> ClientClose
    NEW: 0042B032     E9 A51E0000    JMP GameServ.0042CEDC		-> ProtocolCoreSend (DataSend)

    BYTE CharWingsEx3[] = { 0xE9, 0xA5, 0x1E, 0x00, 0x00 };
    memcpy((int*)0x0042B032,CharWingsEx3,sizeof(CharWingsEx3));
    */

    // PetDurablity - ПРОВЕРЕН (GS,GSCS)
    BYTE FixPetDur[] = { 0xFF, 0x25, 0x50, 0xB5, 0x71, 0x00, 0x90, 0x90 };	// replace with JMP DWORD PTR DS:[71B550]
    memcpy((int*)0x005116D2,FixPetDur,sizeof(FixPetDur));
    *(unsigned int*)0x0071B550 = (unsigned int)FixNewPetDurabDown;

    // View PortPatch - ПРОВЕРЕН (GS,GSCS)
    /*
    ...
    *(unsigned int*)0x0071B050  = (unsigned int)gObjViewportPatchExecute;
    ...
    if(Configs.ClearPersonalShop)
    {
        Utilits.SetByte(0x00504B25+2, 0x00);
    }

    // CSS Skill On Normal (GS)
    ...
    if(Configs.Enable65kStats) // Maximum Stats (65535)
    {
    ...
    *(BYTE*)(0x00560F19) = 0x0F;
    ...
    //Utilits.SetByte(0x004B406F,0xEB);	// Core - error-L1 : CloseClient connect error <--- походу в этом совершенно нет надобности
    Utilits.SetNop(0x0052DE2F,6); //LogAddTD(lMsg.Get(542),lpObj->m_Index,lpObj->AccountID,lpObj->Name,lpObj->Ip_addr);
    Utilits.SetNop(0x0052DE85,6); //LogAddTD(lMsg.Get(543),lpObj->m_Index,lpObj->AccountID,lpObj->Name,lpObj->Ip_addr);
    ...
    //Marlon Quest Fix
    Utilits.SetByte(0x005044B8,0xEB);
    Utilits.SetByte(0x00504519,0xEB);

    // Fix Crash Dump File
    if(Configs.DumpFile == 0)
    {
        Utilits.SetByte(0x004C7041,0xE9);
        Utilits.SetByte(0x004C7042,0x2E);
        Utilits.SetByte(0x004C7043,0x01);
        Utilits.SetByte(0x004C7044,0x00);
        Utilits.SetByte(0x004C7046,0x90);
    }
    ...
    *(BYTE*)(0x005F27C5) = 0x00;
    ...
    /////////////////////////////////
    // Golden Rabbit Fix (GS)
    /////////////////////////////////
    Utilits.SetByte(0x0048130B,0x3D);
    Utilits.SetByte(0x0048130C,0xF6);
    Utilits.SetByte(0x0048130D,0x01);
    Utilits.SetByte(0x0048130E,0x00);
    Utilits.SetByte(0x0041E866,0x3D);
    Utilits.SetByte(0x0041E867,0xF6);
    Utilits.SetByte(0x0041E868,0x01);
    Utilits.SetByte(0x0041E869,0x00);

    if(Configs.CashShopExe)
    {
        /////////////////////////////
        // Fix excelent CashShop (GS)
        /////////////////////////////
	    Utilits.SetNop(0x005F79CF,3);
	    Utilits.SetNop(0x005F79D2,5);
	    Utilits.SetNop(0x005F79D7,2);
	    Utilits.SetNop(0x005F79D9,3);
	    Utilits.SetNop(0x005F79DC,3);
	    Utilits.SetNop(0x005F79DF,3);
	    Utilits.SetNop(0x005F79E2,3);
	    Utilits.SetNop(0x005F79E5,3);
    }
#endif
    ...
    BYTE CharWingsEx[] = {0x8D, 0xBD, 0xEC, 0xFE, 0xFF, 0xFF, 0x57, 0xFF,
                          ...
                          0x07, 0x8D, 0x95, 0xD4, 0xFE, 0xFF, 0xFF, 0x52,
                          0x8D, 0x85, 0xEC, 0xFE, 0xFF, 0xFF, 0x50, 0xE8,
                          0x9C, 0xDD, 0x21, 0x00, 0x83, 0xC4, 0x0C, 0x8B,
                          0x8D, 0xE8, 0xFE, 0xFF, 0xFF, 0x51, 0x8D, 0x95,
                          0xEC, 0xFE, 0xFF, 0xFF, 0x52, 0x8B, 0x85, 0xE0,
                          0xFE, 0xFF, 0xFF, 0x50, 0xE8, 0xA6, 0x72, 0xFD,
                          0xFF, 0x83, 0xC4, 0x0C, 0x5F, 0x5E, 0x5B, 0x8B,
                          0xE5, 0x5D, 0xC3
                         };
    ...
    *(unsigned int*)0x0071B200 = (unsigned int)JGPGetCharInfoEx;
    ...
    // Это фиксы ссылок на оффсеты (все сдвинулось из за хука JGPGetCharInfoEx)
    Utilits.SetByte(0x0042C99A+1,0xA5);//небыло, добавил сам, но он 99.99% верный
    Utilits.SetByte(0x0042CA28+2,0xD5);//был уже, просто короче сделал
    Utilits.SetByte(0x0042CA5A+2,0xA3);//был уже, просто короче сделал OK

    /*
    Beos: хук исправил (краш) но это скорее всего ошибочный хук т.к. в нем нет смысла
    ...
    *(unsigned int*)0x0071B550  = (unsigned int)FixNewPetDurabDown;
    ...
    // View PortPatch
    /*
    00539817     FF75 F8        PUSH DWORD PTR SS:[EBP-8]
    0053981A     FF15 50B07100  CALL DWORD PTR DS:[71B050]
    */
    ...
    *(unsigned int*)0x0071B050  = (unsigned int)gObjViewportPatchExecute;
    ...
    if (Configs.Disable380Opt)
    {
        Utilits.SetNop(0x004FD4C1,5);
    }
    if(Configs.DisableHarmonyOpt)
    {
        Utilits.SetNop(0x004FD4CD,5);
    }
    ...
    /////////////////////////////////
    // Golden Rabbit Fix (CS)
    /////////////////////////////////
    Utilits.SetByte(0x00486FCB,0x3D);
    Utilits.SetByte(0x00486FCC,0xF6);
    Utilits.SetByte(0x00486FCD,0x01);
    Utilits.SetByte(0x00486FCE,0x00);
    Utilits.SetByte(0x004202BF,0x3D);
    Utilits.SetByte(0x004202BF+1,0xF6);
    Utilits.SetByte(0x004202BF+2,0x01);
    Utilits.SetByte(0x004202BF+3,0x00);

    if(Configs.CashShopExe)
    {
        /////////////////////////////
        // Fix excelent CashShop (CS)
        /////////////////////////////
	    Utilits.SetNop(0x0062D00F,3);
	    Utilits.SetNop(0x0062D012,5);
	    Utilits.SetNop(0x0062D017,2);
	    Utilits.SetNop(0x0062D019,3);
	    Utilits.SetNop(0x0062D01C,3);
	    Utilits.SetNop(0x0062D01F,3);
	    Utilits.SetNop(0x0062D022,3);
	    Utilits.SetNop(0x0062D025,3);
    }
